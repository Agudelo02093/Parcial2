...
function setup() {
    ...
    socket.on('arduino-message', (arduinoMessage) => {
        potentiometerValue = arduinoMessage.potentiometerValue;
        player.x = map(potentiometerValue, 0, 1023, 0, windowWidth);
        });
...
socket.on('arduino-message', (arduinoMessage) => {
    ...
    //1.1
    player.x = arduinoMessage.potentiometer * (window.innerWidth / 1023);
    //1.2
    if (arduinoMessage.button === 'B') {
        ball.speed += 1;
    }
    //1.3
    if (arduinoMessage.button === 'A') {
        player.width -= 5;
    }   
...
async function sendScore(point) {
    ...
    // send a character 'S' to the Arduino
    socket.emit('arduino-message', 'S');
};
...
async function gameResult() {
    ...
    // send a character 'L' to the Arduino
    socket.emit('arduino-message', 'L');
}